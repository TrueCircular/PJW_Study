...
public:
	...
	virtual int			SendPacket(TUser* pUser, const UPACKET& PacketMsg);
	virtual int			SendPacket(TUser* pUser, const std::stringstream & SendStream);
	void				SendPacket(TUser * pUser, size_t iSendByte);
	virtual int			DirectSendPacket(TUser* pUser, const UPACKET& PacketMsg);
	virtual int			DirectSendPacket(TUser* pUser, const std::stringstream & SendStream);
	void				DirectSendPacket(TUser * pUser, size_t iSendByte);
	// broadcast pool save -> run ->send
	virtual void		SendBroadcastPool(const UPACKET& sendmsg);
	virtual void		SendBroadcastPool(const std::stringstream & SendStream);
	virtual void		SendBroadcastPacket();
	virtual void		Broadcast( const T_PACKET& pSendUser);
	virtual void		Broadcast( const UPACKET& pSendUser);
	void				Broadcast( const std::stringstream & SendStream);
	void				Broadcast( tGUID tGuidExcept, const std::stringstream & SendStream);
public:
